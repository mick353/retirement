...
- Election regression covers all eight valid basis/election combinations across Command Centre, Atlas and V23. Both 100% cases are checked for zero voluntary portfolio draw, exact PSS-surplus routing to Pool C and source-specific FAS/TBC treatment.
- Projection-basis regression covers the shared URL, local-state schema and all three interfaces. Both source sets expose four direct elections. The 6% / 5% / 3% set exposes 2.91% and 1.94% real equivalents; its later 100% estimate retains its $162,485.36 FAS while the 1 September elections retain $162,380.20.
- The source boundary is build-blocking at election level: prudent 100% must remain $147,713.96 gross / $135,575.70 net with no PSS lump and may not borrow current-basis values. The 65/35 and 70/30 provider one-cent component residuals are accepted but not rewritten.
- Wash regression verifies direct PSS components and source-limited maxima of 6, 5, 4 and 0 cycles for the current basis; 5, 5, 4 and 0 for the prudent basis; plus 5 for Rail A.
